...
text(1.2e4, 140, 'asymmetry = 100 (two orders)', ...
     'FontSize',8,'Color',[0.30 0.30 0.30]);
text(1.2e4, 14,  'asymmetry = 10', ...
     'FontSize',8,'Color',[0.45 0.45 0.45]);

xlabel('Replacement cost $C$ (USD)','Interpreter','latex');
ylabel('Asymmetry $\tilde{R}^{col}/\tilde{R}^{ind}$','Interpreter','latex');
legend('show','Location','northeast','FontSize',8,'Box','off');
ylim([1e-3, 1e7]);
xlim([1e4, 1e10]);
grid on;
set(gca,'FontSize',10,'Box','on','GridAlpha',0.15,'Layer','top');
