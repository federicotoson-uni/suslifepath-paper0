...
text(ax1, 0.05, ax1.YLim(2)*0.95, 'R=1 (parity)', ...
     'Rotation',90,'VerticalAlignment','top','FontSize',9,'Color',[.3 .3 .3]);
% Case study markers
...
for c = 1:3
    text(ax1, log10(case_asym(c)), yl(2)*(0.65 + 0.10*c), ...
         ...
         'Color', case_colors(c,:), 'FontSize',9, 'FontWeight','bold');
end
...
xlim(ax2, [200 2000]);
ylim(ax2, [1e-3 1e4]);
legend(ax2, h_legend, 'Location','best','FontSize',9);
